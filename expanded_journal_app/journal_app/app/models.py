...
class JournalEntry(db.Model):
    ...
    journal_title: so.Mapped[str] = so.mapped_column(sa.String(1024))
    journal_entry: so.Mapped[str] = so.mapped_column(sa.String(64000))
    ...
    def __repr__(self):
        return '<JournalEntry {}>'.format(self.journal_title)

class Media(db.Model):
    media_id: so.Mapped[int] = so.mapped_column(primary_key=True)
    entry_id: so.Mapped[int] = so.mapped_column(sa.ForeignKey(JournalEntry.entry_id))
    ...
    def __repr__(self):
        return '<Media {}>'.format(self.media_format)    
...
class JournalEntryTag(db.Model):
    ...
    def __repr__(self):
        return '<JournalEntryTag {}>'.format(self.entry_id)
